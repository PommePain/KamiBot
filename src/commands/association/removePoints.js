    ...
    async execute(interaction) {
        try {
            if (interaction.member.permissions.has("MANAGE_GUILD")) {
                ...
                if (user !== null) {
                    user.current_points = (user.current_points - amount >= 0) ? user.current_points - amount : 0;
                    await user.save();
    
                    await interaction.reply({ content: `J'ai bien retiré ${amount} points à ${user.discord_username}`, ephemeral: false });
                } else {
            ...
            console.log("Error at removePoints execution : ", error);
        }
    } 
}